...
# -*- coding: utf-8 -*-
"""
generar_presentacion_v4.py
Version 4:3: adaptada para relacion de aspecto 4:3 (1024x768).

Uso:
    python generar_presentacion_v4.py
    Abrir presentacion_saas_v4.html en el navegador (F11 pantalla completa).
...
.stat-num {
  display: block; font-size: clamp(24px, 4vw, 40px);
...
.final-title  { font-size: clamp(38px, 8vw, 72px); font-weight: 900; color: #ffffff; }
...
# ──────────────────────────────────────────────────────────────────────────────
def main():
    print("Generando presentacion v4 (4:3)...")
    slides = build_slides()
    slides_html = "\n".join(slides)
...
{slides_html}
  ...
  <script>{JS}</script>
    ...
    out = BASE_DIR / "presentacion_saas_v4.html"
