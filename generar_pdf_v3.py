...
"""
generar_pdf_v3.py
Convierte la presentacion v3 a PDF usando Playwright (Chromium headless).
Cada slide queda en una pagina 16:9 con fondo completo.

Uso:
    python generar_pdf_v3.py
    Genera: presentacion_saas_v3.pdf
"""
...
try:
    import generar_presentacion_v3 as v3
except ImportError as e:
    print(f"Error importando v3: {e}")
    sys.exit(1)

# ──────────────────────────────────────────────────────────────────────────────
# CSS que sobreescribe el layout de presentacion para impresion plana
# ──────────────────────────────────────────────────────────────────────────────
PRINT_CSS = """
@page {
  size: 1280px 720px;
...
async def main():
    ...
    print("Generando HTML de impresion...")
    slides = v3.build_slides()
    slides_html = "\n".join(slides)
    n_slides = len(slides)
...
<head>
  <meta charset="UTF-8">
  <title>Simulacion SaaS v3 - PDF</title>
  <style>{v3.CSS}</style>
...
{slides_html}
    ...
    out = BASE_DIR / "presentacion_saas_v3.pdf"

    print(f"Iniciando Chromium ({n_slides} slides)...")
    try:
